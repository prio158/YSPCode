...
static int read_packet(void *opaque, uint8_t *buf, int buf_size)
{
    ...
    // 这里不能把这个packet都丢入buf，而是只把数据区域丢入。
                   ...
                   FILE *outfile)
{
    ...
    while (ret >= 0)
    {
        ...
        /**
           ...
           LLLLLLRRRRRRLLLLLLRRRRRRLLLLLLRRRRRRL...（每个LLLLLLRRRRRR为一个音频帧，一个音频帧包含1024个sample数）
...
int main(int argc, char **argv)
{
    ...
    // avformat_open_input读取数据的时候分析完，不会将数据丢掉
    // av_read_frame的时候才会把数据读走
