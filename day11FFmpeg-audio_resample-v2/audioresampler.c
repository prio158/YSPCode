...
static int init_resampled_data(audio_resampler_t *resampler)
{
    ...
    int linesize = 0;
    // 获取输入或输出的通道数、样本数后、format后，给输入源或输出源分配好内存空间buf。
    // resampler->resampled_data[channels][smaples]
...
audio_resampler_t *audio_resampler_alloc(const audio_resampler_params_t resampler_params)
{
    ...
    audio_resampler->cur_pts = AV_NOPTS_VALUE;   // 初始化设置
    audio_resampler->start_pts = AV_NOPTS_VALUE; // 初始化设置
...
int audio_resampler_send_frame(audio_resampler_t *resampler, AVFrame *frame)
{
    ...
    const int dst_nb_samples = av_rescale_rnd(swr_get_delay(resampler->swr_ctx, resampler->resampler_params.src_sample_rate) + src_nb_samples,
                                              resampler->resampler_params.dst_sample_rate, resampler->resampler_params.src_sample_rate, AV_ROUND_UP);
    // 初识化的resampled_data_size是2048，这个默认值，所以需要重新分配
...
{
    // 这个逻辑就是当nb_samples=0的时候，进行flush，冲刷fifo缓存。
