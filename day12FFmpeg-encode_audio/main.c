...
static int encode(AVCodecContext *ctx, AVFrame *frame, AVPacket *pkt, FILE *output)
{
    ...
    while (ret >= 0) {
        ...
        //receive出来的packet数据（编码好的）是没有包含aac header的，所以这里需要手动加。
        size_t len = 0;
...
/*
 * 这里只支持2通道的转换：packed LRLRLRLR.. ---> planar LLLL...RRRR...
*/
...
int main(int argc, char **argv)
{
    ...
    printf("2 frame_size:%d\n\n", codec_ctx->frame_size); // 决定每次到底送多少个采样点,它是由编码器决定，而不是用户设置的
    // 打开输入和输出文件
...
//            continue;
        ...
        */
        ...
        if(AV_SAMPLE_FMT_S16 == frame->format) {
            ...
            //将pcm_temp_buf[0] = LLLL....1024..LRRRR...1024....R
            ret = av_samples_fill_arrays(frame->data, frame->linesize,
