...
static void open_audio(AVFormatContext *oc, AVCodec *codec, OutputStream *ost, AVDictionary *opt_arg)
{
    ...
    // 分配送给信号生成PCM的帧, 并申请对应的buffer,这个数据是由代码生成的PCM数据
...
static AVFrame *get_video_frame(OutputStream *ost)
{
    ...
    // 如果 ost->next_pts*codec_ctx->time_base == STREAM_DURATION*AVRational){1, 1} 就返回1，else:-1
...
int main(int argc, char **argv)
{
    ...
    AVOutputFormat *fmt; // 输出文件容器格式, 封装了复用规则(格式封装)，AVInputFormat则是封装了解复用规则（格式解封装）
    AVFormatContext *oc;
    ...
    // audio AVstream->base_time = 1/44100, video AVstream->base_time = 1/25，pts的单位：time_base
    /* 写头部. 到底做了什么操作呢？ 对应steam的time_base被改写 ,改写的这个操作 和封装格式有关系*/
    ...
    while (encode_video || encode_audio)
    {
        ...
        {
            printf("\nwrite_audio_frame\n");
            encode_audio = !write_video_frame(oc, &audio_st);
        }
    }
