...
int main(int argc, char **argv)
{
    ...
    int pcm_frame_size = av_get_bytes_per_sample((AVSampleFormat)pcm_sample_format) * pcm_channels * audio_encoder.GetFrameSize();
    if (pcm_frame_size <= 0)
    {
    ...
    double audio_pts = 0;
    double video_pts = 0;
    double audio_frame_duration = 1.0 * audio_encoder.GetFrameSize() / pcm_sample_rate * audio_time_base; //乘上audio_time_base，是为了换算成us
    double video_frame_duration = 1.0 / yuv_fps * video_time_base; //乘上video_time_base，就是将单位换算成us
