...
int CFlvParser::Parse(uint8_t *pBuf, int nBufSize, int &nUsedLen)
{
    ...
    while (1)
    {
        CheckBuffer(15); // nPrevTagSize(4字节) + Tag header(11字节)
        int nPrevSize = ShowU32(pBuf + nOffset);
        nOffset += 4; // nPrevTagSize占4个字节，所以加4
        // Tag
        ...
        nOffset += (11 + pTag->_header.nDataSize); // tag header size + data size
...
CFlvParser::CVideoTag::CVideoTag(TagHeader *pHeader, uint8_t *pBuf, int nLeftLen, CFlvParser *pParser)
{
    ...
    // Vedio Tag Data的第一个字节是Stream Header，占一个字节，
    // 其中帧类型占高4位，编码类型占低4位
    _nFrameType = (pd[0] & 0xf0) >> 4; // 帧类型 ， >>4 是位了将高4位换算成十进制
...
{
    uint8_t *pd = _pTagData;
    //audioObjectType就是aac的profile - 1
...
int CFlvParser::CAudioTag::ParseRawAAC(CFlvParser *pParser, uint8_t *pTagData)
{
    ...
    // 数据长度 跳过tag data的第一个第二字节 (StramHeader AAC 占一个字节，AACPacketType占一个字节)
    int dataSize = _header.nDataSize - 2; // 减去两字节的 audio tag data信息部分

    //制作元数据，根据ADTS的格式来制作元数据ADTS Header =  adts_fixed_header + adts_variable_header    
    WriteU64(bits, 12, 0xFFF);                 //syncword  12位，代表一个ADTS帧的开始
    WriteU64(bits, 1, 0);                      //ID,0代表MPEG-4，1位
    WriteU64(bits, 2, 0);                      //Layer,总是'00'，2位
    WriteU64(bits, 1, 1);                      //protection_absent:表示是否有校验，1代表no CRC
    WriteU64(bits, 2, _aacProfile - 1);        //profile 
    WriteU64(bits, 4, _sampleRateIndex);       //采样率下标
    WriteU64(bits, 1, 0);                      //private_bit，1位，默认都是0
    WriteU64(bits, 3, _channelConfig);         //声道数，3位
    WriteU64(bits, 1, 0);                      //original_copy,1位
    WriteU64(bits, 1, 0);                      //home，1位
    WriteU64(bits, 1, 0);                      //从这1位开始，是adts_variable_header(),它是copyright_identification_bit,1位
    WriteU64(bits, 1, 0);                      //copyright_identification_start，1位
    WriteU64(bits, 13, 7 + dataSize);          //acc_frame_length = (protection_absent==1?7:9)+size(AACFrame),13位
    WriteU64(bits, 11, 0x7FF);                 //adts_buffer_fullness:0x77代表码率是可变的码流
    WriteU64(bits, 2, 0);                      //number_of_raw_data_blocks_in_frame = 0 ,说明ADTS帧中有一个AAC数据块
    // WriteU64执行为上述的操作，最高的8bit还没有被移位到，实际是使用7个字节
    _nMediaLen = 7 + dataSize;
    _pMedia = new uint8_t[_nMediaLen];
    uint8_t p64[8]; //将上面uint64类型的bits转为16进制
    ...
    memcpy(_pMedia + 7, pTagData + 2, dataSize); // AAC body，pTagData[1]是AACPacketType，所以从pTagData[2]开始，后面才是AAC ES(raw_data_block)
...
CFlvParser::CMetaDataTag::CMetaDataTag(TagHeader *pHeader, uint8_t *pBuf, int nLeftLen, CFlvParser *pParser)
{
    ...
    uint8_t *pd = _pTagData;
    m_amf1_type = ShowU8(pd + 0);  // Type 1个字节
    m_amf1_size = ShowU16(pd + 1); // Value_Size  2个字节
...
int CFlvParser::CMetaDataTag::parseMeta(CFlvParser *pParser)
{
    ...
    uint32_t arrayLen = 0;
    uint32_t offset = 13; // Type（1 byte） + Value_Size（3 byte）+ Value（10 byte） 占用13字节
    ...
    {
        arrayLen = ShowU32(pd + offset); //ECMAArrayLength代表要下面解析的字段个数
        offset += 4; // 跳过 [ECMAArrayLength]占用的字节 4个字节
        printf("ArrayLen = %d\n", arrayLen);
    }
    else
    {
    ...
    for (uint32_t i = 0; i < arrayLen; i++)
    {
        ...
        nameLen = ShowU16(pd + offset); // 读取名称字段长度
        offset += 2;                    // 跳过2字节字段长度
...
CFlvParser::Tag *CFlvParser::CreateTag(uint8_t *pBuf, int nLeftLen)
{
    // 开始解析Tag Header
    TagHeader header;
    header.nType = ShowU8(pBuf + 0);       // 类型（1个字节）
    header.nDataSize = ShowU24(pBuf + 1);  // 标签body的长度 (3个字节)
    header.nTimeStamp = ShowU24(pBuf + 4); // 时间戳 低24bit (3个字节)
    header.nTSEx = ShowU8(pBuf + 7);       // 时间戳的扩展字段, 高8bit
    header.nStreamID = ShowU24(pBuf + 8);  // 流的id

    // header.nTSEx << 24 代表高8位，header.nTimeStamp低24位
    // eg： TimeStamp            TSEx
    //     0000 ... 0 (24位)   1111 1111
    //  TSEx << 24    TimeStamp
    //  1111 1111    0000 ... 0 (24位)
    //    高8位          低24位
...
int CFlvParser::CVideoTag::ParseH264Tag(CFlvParser *pParser)
{
    ...
    */
    int nAVCPacketType = pd[1]; // Vedio Tag Data的第二个字节开始就是数据部分了：AVCPacketType
...
int CFlvParser::CVideoTag::ParseH264Configuration(CFlvParser *pParser, uint8_t *pTagData)
{
    ...
    // NalUnit长度表示占用的字节
    pParser->_nNalUnitLength = (pd[offset + 4] & 0x03) + 1; // lengthSizeMinusOne 9 = 5 + 4

    int sps_size, pps_size;
    // sps（序列参数集）的长度
    sps_size = CFlvParser::ShowU16(pd + offset + 6); // sequenceParameterSetLength 11 = 5 + 6
    // pps（图像参数集）的长度
    pps_size = CFlvParser::ShowU16(pd + offset + 6 + (2 + sps_size) + 1); // pictureParameterSetLength

    // 元数据的长度
    _nMediaLen = 4 + sps_size + 4 + pps_size; // 添加start code
    _pMedia = new uint8_t[_nMediaLen];
    // 保存元数据
    memcpy(_pMedia, &nH264StartCode, 4); //在头部4个字节添加startCode
    memcpy(_pMedia + 4, pd + 11 + 2, sps_size); //将tag Data中的sps拷过去
    memcpy(_pMedia + 4 + sps_size, &nH264StartCode, 4); //添加startCode
    memcpy(_pMedia + 4 + sps_size + 4, pd + 11 + 2 + sps_size + 2 + 1, pps_size); //将tag Data中的pps拷过去
...
{
    uint8_t *pd = pTagData; //pd 指向Tag Data，pd[0]就是Stream Header字段，pd[1]是AVCPacketType
    int nOffset = 0;
    ...
    while (1)
    {
        ...
        // 一个tag可能包含多个nalu, 所以每个nalu前面有NalUnitLength个字节表示每个nalu的长度
        int nNaluLen;
